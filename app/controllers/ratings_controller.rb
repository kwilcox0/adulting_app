class RatingsController < ApplicationController
  ...
  def index
    ...
    @ratings = @q.result(:distinct => true).includes(:user, :video).page(params[:page]).per(10)
  ...
  def destroy_row_from_video
    @rating = Rating.find(params.fetch("id_to_remove"))

    @rating.destroy

    redirect_to("/videos/#{@rating.video_id}", notice: "Rating deleted successfully.")
  end
